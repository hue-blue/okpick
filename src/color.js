var Color = (function (exports) {
    ...
    if (coord.indexOf(".") > 0) {
      ...
      if (!(coordName in space.coords)) {
        throw new ReferenceError(
          `Color space "${space.name}" has no "${coordName}" coordinate.`
        );
      }
  ...
  class Color$1 {
    ...
    constructor(...args) {
      ...
      if (
        ...
        if (hasDOM && args[0].indexOf("--") === 0) {
          // CSS variable
          let root =
            arguments[1] && arguments[1].nodeType === 1
              ? arguments[1]
    ...
    toString({
      ...
      let strAlpha =
        this.alpha < 1 ? ` ${commas ? "," : "/"} ${this.alpha}` : "";
      ...
      if (fallback) {
        ...
        let fallbacks = Array.isArray(fallback)
          ? fallback.slice()
          : Color$1.defaults.fallbackSpaces;
    ...
    static chromaticAdaptation(W1, W2, XYZ, options = {}) {
      ...
      if (!env.M) {
        if (env.W1 === Color$1.whites.D65 && env.W2 === Color$1.whites.D50) {
          ...
          env.W1 === Color$1.whites.D50 &&
          env.W2 === Color$1.whites.D65
        ) {
          // env.M = [
    ...
    static parse(str) {
      ...
      if (env.parsed) {
        ...
        if (name === "rgb" || name === "rgba") {
          let args = env.parsed.args.map((c, i) =>
            i < 3 && !c.percentage ? c / 255 : +c
          );
          ...
          let space = Object.values(Color$1.spaces).find(
            (space) => (space.cssId || space.id) === spaceId
          );
          ...
          if (space) {
            ...
            throw new TypeError(
              `Color space ${spaceId} not found. Missing a plugin?`
            );
          }
        }
      }
    ...
    static defineSpace({ id, inherits }) {
      ...
      if (!space.fromXYZ && !space.toXYZ) {
        ...
        if (connectionSpace) {
          ...
          throw new ReferenceError(
            `No connection space found for ${space.name}.`
          );
        }
      }
      ...
      Object.defineProperty(Color$1.prototype, id, {
        ...
        get() {
          ...
          return new Proxy(ret, {
            has: (obj, property) => {
              return (
                coordNames.includes(property) || Reflect.has(obj, property)
              );
            },
  ...
  Color$1.defineSpace({
    ...
    toLinear(RGB) {
      return RGB.map(
        (val) => Math.pow(Math.abs(val), 563 / 256) * Math.sign(val)
      );
    },
    toGamma(RGB) {
      return RGB.map(
        (val) => Math.pow(Math.abs(val), 256 / 563) * Math.sign(val)
      );
    },
  ...
  Color$1.defineSpace({
    ...
    LMStoRec2020_M: [
      ...
      [
        -0.025646662911506476363,
        -0.099240248643945566751,
        1.1248869115554520431
      ]
    ],
    LMStoXYZ_M: [
  ...
  Color$1.defineSpace({
    ...
    white: (Color$1.whites.ACES = [
      0.32168 / 0.33767,
      1.0,
      (1.0 - 0.32168 - 0.33767) / 0.33767
    ]),
    // Similar to D60
    ...
    if (maxDeltaE > 0) {
      ...
      while (maxDelta > maxDeltaE) {
        ...
        for (let i = 1; i < ret.length && ret.length < maxSteps; i++) {
          ...
          maxDelta = Math.max(
            maxDelta,
            color.deltaE(prev.color),
            color.deltaE(cur.color)
          );
  ...
  Color$1.range = function (color1, color2, options = {}) {
    if (isRange(color1)) {
      ...
      return Color$1.range(...range.rangeArgs.colors, {
        ...range.rangeArgs.options,
        ...options
      });
    }
    ...
    if (space) {
      ...
      space =
        Color$1.spaces[Color$1.defaults.interpolationSpace] || color1.space;
    }

    outputSpace = outputSpace
      ? Color$1.space(outputSpace)
      : color1.space || space;
    ...
    if (space.coords.hue && space.coords.hue.isAngle) {
      ...
      [color1[space.id].hue, color2[space.id].hue] = adjust(arc, [
        color1[space.id].hue,
        color2[space.id].hue
      ]);
    }
  ...
  Color$1.prototype.deltaE2000 = function (
    sample,
    { kL = 1, kC = 1, kH = 1 } = {}
  ) {
    let color = this;
